...
export async function resolveImplicitProviders(params: {
  ...
  // OpenRouter: always add so openrouter/* model refs resolve (key from env or profile at request time)
  const openrouterKey =
  ...
  providers.openrouter = {
    ...
    models: [], // openrouter/* refs resolved via fallback in resolveModel
  };

  // Z.AI (GLM): add when ZAI_API_KEY or zai auth profile exists so zai/* models (e.g. glm-4.6v-flash) resolve
  const zaiKey =
  ...
  if (zaiKey) {
    providers.zai = {
      ...
      api: "openai-completions",
      apiKey: zaiKey,
      models: [], // zai/* refs resolved via fallback in resolveModel
    };
  }
