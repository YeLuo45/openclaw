...
**Debate workflow:**
1. `init --mode debate -g "question"` — creates project with a question/topic
2. `add-debater [--role "perspective"]` — add agents as debaters, each with an optional role/perspective
3. `round start` — generates dispatch info for round 1 (each debater gets the question + their role)
4. After collecting responses: `round collect "response"` — save each debater's response
5. `round cross-review` — generates cross-review prompts (each debater reviews others' responses)
6. After cross-review: `round collect "review"` — save reviews
7. `round synthesize` — outputs all positions + reviews for final synthesis
...
The `round cross-review` command should output structured prompts for each debater:
