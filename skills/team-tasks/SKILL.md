...
All commands use: `python3 <skill-dir>/scripts/task_manager.py [args]`
...
|---------|------|-------|-------------|
| `init` | both | `init <project> -g "goal" [-m linear\|dag\|debate]` | Create project |
...
| `assign` | both | `assign <project> <stage> "desc"` | Set task description |
| `update` | both | `update <project> <stage> <status>` | Change status |
...
| `log` | both | `log <project> <stage> "msg"` | Add log entry |
| `result` | both | `result <project> <stage> "output"` | Save output |
| `reset` | both | `reset <project> [stage] [--all]` | Reset to pending |
| `history` | both | `history <project> <stage>` | Show log history |
...
```bash
TM="python3 scripts/task_manager.py"
$TM init security-review --mode debate -g "Review auth module for security vulnerabilities"
...
# Collect: round <project> collect <agent-id> "response text"
$TM round security-review cross-review
# Collect cross-reviews, then:
...
| | Mode A (linear) | Mode B (dag) | Mode C (debate) |
|---|---|---|---|
| **When** | Sequential tasks, simple flows | Parallel workstreams, complex deps | Code review, architecture decisions |
| **Dispatch** | One at a time, auto-advance | Multiple simultaneous, dep-driven | Same question to N agents, then cross-review |
| **Setup** | `init -p agents` (one command) | `init -m dag` + `add` per task | `init -m debate` + `add-debater` + `round` |
| **Best for** | Bug fixes, simple features | Large features, spec-driven dev | Competing hypotheses, security review |

## Data Location

Task files: `TEAM_TASKS_DIR/<project>.json` (default `/home/ubuntu/clawd/data/team-tasks/`). Override with env `TEAM_TASKS_DIR`.
...
### DAG: Dependencies must exist before referencing
Add dependency tasks first, then tasks that depend on them.

### Debate: Cannot add debaters after rounds start
Add all debaters with `add-debater` before running `round start`.
